...
params = {'max_depth': 6, 'eta': 1, 'silent': 1, 'objective': 'binary:logistic', 'scale_pos_weight': 0.67,
...
predictions = (((rdfclf.predict_proba(Z)[:, 1]) + gbm.predict(xgb.DMatrix(Z))) / 2 > 0.5).astype(int)
